...
function UserProfile(props) {
    ...
    useEffect(() => {
        ...
        if (props.location.state) {
            ...
            firestore.collection("user").doc(props.location.state).get().then((doc) => {
                if (doc.exists) {
    ...
    const getFollowStatus = async () => {
        await firestore.collection('follows').where("followingID", "==", `${props.location.state}`).onSnapshot((snapshot) => {
            setFollowers(snapshot.docs.length);
        });

        if (currentUser) {
            await firestore.collection('follows').where('uid', "==", `${currentUser.uid}`).where('followingID', "==", `${props.location.state}`).onSnapshot((snapshot) => {
                    ...
                    <div className="sideprofile">
                        <div className="profile">
                            ...
                            <div className="otherinfo">
                                ...
                                {currentUser ?  <button className="follow" onClick={changeStatus}>{followStatus ? "Following" : "Follow"}</button> : <></> }
